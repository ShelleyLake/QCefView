...
QCefViewImplement::~QCefViewImplement() {
  LOG(INFO) << "Exit Trace: ~QCefViewImplement enter";
  LOG(INFO) << "Exit Trace: ~QCefViewImplement leave";
}
...
void QCefViewImplement::OnImeCompositionRangeChanged(
...
void QCefViewImplement::navigateToUrl(const QString &url) {
  ...
  if (pQCefViewHandler_ && pQCefViewHandler_->GetBrowser() &&
      pQCefViewHandler_->GetBrowser()->GetMainFrame()) {
...
bool QCefViewImplement::sendEventNotifyMessage(int frameId, const QString &name,
      ...
      if (pMsg->message == WM_SYSCHAR || pMsg->message == WM_SYSKEYDOWN ||
        ...
        if (pCefView_->isActiveWindow() && pCefView_->hasFocus()) {
          pCefUIEventWin_->OnKeyboardEvent(pMsg->message, pMsg->wParam, pMsg->lParam);
               ...
               pMsg->message == WM_MOUSEMOVE || pMsg->message == WM_MOUSELEAVE ||
...
QCefView *QCefViewImplement::getCefView() { return pCefView_; }
...
void QCefViewImplement::paintEvent(QPaintEvent *event) {
  ...
  if (pDrawImageParam) {
    ...
    qInfo() << "paintEvent, QImage: " << image.rect() << "MemImage:" << pDrawImageParam->imageWidth << "x"
            << pDrawImageParam->imageHeight << ", CefView: " << pCefView_->rect();

    paint.drawImage(QRectF({0, 0, (qreal)image.width() / deviceScaleFactor_,
                            (qreal)image.height() / deviceScaleFactor_}),
                    image);
  }
  pQCefViewHandler_->releaseImage();
}

float QCefViewImplement::deviceScaleFactor() { return deviceScaleFactor_.load(); }

void QCefViewImplement::setFPS(int fps) { fps_ = fps; }

int QCefViewImplement::fps() const { return fps_; }
