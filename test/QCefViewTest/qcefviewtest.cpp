...
QCefViewTest::QCefViewTest(QWidget *parent)
  ...
  connect(ui.lineEditUrl, &QLineEdit::returnPressed,
          [this]() { ui.widgetCef->navigateToUrl(ui.lineEditUrl->text()); });

  connect(ui.pushButtonGoBack, &QPushButton::clicked, [this]() { ui.widgetCef->browserGoBack(); });

  connect(ui.pushButtonGoForward, &QPushButton::clicked,
          [this]() { ui.widgetCef->browserGoForward(); });
  ...
  connect(ui.pushButtonClose, &QPushButton::clicked, [this]() { ui.widgetCef->closeBrowser(); });
  ...
  connect(
      ui.widgetCef, &QCefView::loadingStateChanged, this,
  ...
  connect(
      ui.widgetCef, &QCefView::invokeMethodNotify, this,
      [this](int browserId, int frameId, const QString &method, const QVariantList &arguments) {
        qInfo() << "[invokeMethodNotify] "
                << "browserId: " << browserId << ", frameId: " << frameId << ", method: " << method
                << ", arguments: " << arguments;
      },
      Qt::QueuedConnection);

  connect(
      ui.widgetCef, &QCefView::cefUrlRequest, this,
      [this](const QString &url) { qInfo() << "[cefUrlRequest] " << url; }, Qt::QueuedConnection);

  connect(
      ui.widgetCef, &QCefView::cefQueryRequest, this,
      [this](const QCefQuery &query) {
        qInfo() << "[cefQueryRequest] "
                << "id: " << query.id() << ", reqeust:" << query.reqeust()
                << ", response: " << query.response() << ", result: " << query.result()
                << ", error: " << query.error();
      },
      Qt::QueuedConnection);
  ...
  //                            "/QCefViewTestPage.html");
  ui.widgetCef->navigateToUrl("www.baidu.com");
}
